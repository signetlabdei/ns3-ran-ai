...
public:
   ...
   */
  void TxBurst (uint32_t nodeId, Ptr<const Packet> burst, const Address &from, const Address &to, const SeqTsSizeFragHeader &header);
...
private:
  ...
  std::map<uint32_t, uint32_t> m_rxBursts; //!< number of bursts received in a specific epoch per node ID
  std::map<uint32_t, uint64_t> m_rxData; //!< number of bytes received in a specific epoch per node ID

  std::map<uint32_t, Ptr<MinMaxAvgTotalCalculator<uint64_t>>> m_delay; //!< delay statistics calculator for a specific epoch, per node ID
